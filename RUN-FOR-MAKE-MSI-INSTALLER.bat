...
set NAME=CourseManager
set MAIN_JAR=%NAME%-jar-with-dependencies.jar
set MAIN_CLASS=com.coursemanager.Launcher
set ICON_PATH=D:\Projects\IntelliJ_IDEA\CourseManager\src\main\resources\Icons\AppIcon.ico

set INPUT_DIR=build_temp
set OUTPUT_DIR=VERSIONS\cman-%VERSION%
...
jpackage ^
  ...
  --app-version %VERSION% ^
  --vendor "Ha3yxa" ^
  --description "CourseManager %VERSION% - course management application" ^
  --icon "%ICON_PATH%" ^
  --win-menu ^
  --win-shortcut ^
  --win-dir-chooser ^
